...
export default function SettingsScreen() {
  ...
  const userId = `usr_${(email || 'guest').split('@')[0]}_${Math.abs((email || '').length * 31)}`;
  ...
  return (
    <Screen>
      ...
      <Card elevated style={styles.profile}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.rowTitle}>{email ?? 'Signed in User'}</Text>
          <Text style={styles.rowSub}>ID: {userId}</Text>
      ...
      <Card style={styles.settingCard}>
        <View style={styles.settingRow}>
          <View style={[styles.iconWrap, { backgroundColor: colors.income + '22' }]}>
            <Ionicons name="server" size={18} color={colors.income} />
          </View>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.settingDesc}>Encrypted statements & fraud baselines</Text>
          </View>
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Supabase Database Setup</Text>
              <Pressable onPress={() => setSupabaseModalVisible(false)} hitSlop={10}>
                <Ionicons name="close-circle" size={24} color={colors.textMuted} />
              </Pressable>
            </View>
            <Text style={styles.modalDesc}>
              Fin connects to Supabase for secure cloud synchronization. Here is how to configure your project credentials:
            </Text>

            <View style={styles.stepBox}>
              <Text style={styles.stepTitle}>1. Project URL</Text>
              <Text style={styles.stepDesc}>Found in Supabase Dashboard → Project Settings → API → Project URL.</Text>
            </View>

            <View style={styles.stepBox}>
              <Text style={styles.stepTitle}>2. Anon / Public Key</Text>
              <Text style={styles.stepDesc}>Found in Supabase Dashboard → Project Settings → API → Project API Keys (anon key).</Text>
            </View>

            <View style={styles.stepBox}>
              <Text style={styles.stepTitle}>3. Environment Variables</Text>
              <Text style={styles.stepDesc}>Configured in backend `.env` as `SUPABASE_URL` and `SUPABASE_ANON_KEY`.</Text>
            </View>

            <Pressable style={[styles.modalBtn, { marginTop: spacing.md }]} onPress={() => setSupabaseModalVisible(false)}>
